...
interface BannerConfig {
  ...
  subheadlinePosition: SubheadlinePosition;
...
const initialBannerConfig: BannerConfig = {
    imageUrl: '',
    ...
    titleStyle: { type: 'solid', color1: '#FFFFFF', color2: '#FFD700', direction: 'to right' },
    titleFontFamily: FONT_OPTIONS[0].family,
    titleFontSize: 64,
    subheadlinePosition: 'below',
    subheadlineStyle: { type: 'solid', color1: '#FFFFFF', color2: '#1E90FF', direction: 'to right' },
    subheadlineFontFamily: FONT_OPTIONS[1].family,
    subheadlineFontSize: 32,
    logoUrl: null,
    logoPosition: { x: 0.98, y: 0.98 },
    logoScale: 0.15,
};
...
const App: React.FC = () => {
  ...
  // Undo/Redo state
  const [history, setHistory] = useState<BannerConfig[]>([]);
  ...
  useEffect(() => {
    if (isRestoring || !bannerConfig.imageUrl) return;
    ...
    historyTimeoutRef.current = window.setTimeout(() => {
        ...
        if (newHistory.length > 0 && JSON.stringify(newHistory[newHistory.length - 1]) === JSON.stringify(bannerConfig)) {
            return;
        }

        newHistory.push(bannerConfig);
...
}, [bannerConfig, history, historyIndex, isRestoring]);
  ...
  const handleUndo = useCallback(() => {
    if (canUndo) {
        ...
        setBannerConfig(history[newIndex]);
  ...
  const handleGeneratePrompt = useCallback(async () => {
    ...
    try {
      const { imagePrompt: genPrompt, bannerTitle: genTitle, bannerSubheadline: genSubheadline } = await generateContentFromUrl(url);
      setImagePrompt(genPrompt);
      setBannerConfig(s => ({...s, bannerTitle: genTitle, bannerSubheadline: genSubheadline }));
    } catch (err) {
  ...
  const handleGenerateImage = useCallback(async () => {
    if (!imagePrompt) {
     ...
     if (bannerConfig.showBannerTitle && !bannerConfig.bannerTitle) {
      setError('A banner title is required when "Show on Banner" is checked.');
      return;
    }
     if (bannerConfig.showSubheadline && !bannerConfig.bannerSubheadline) {
    ...
    try {
      ...
      setBannerConfig(newBannerState);
      setHistory([newBannerState]);
      setHistoryIndex(0);
  ...
  }, [imagePrompt, bannerConfig, aspectRatio, customDimensions]);

  const handleEditImage = useCallback(async () => {
    if (!bannerConfig.imageUrl || !editPrompt.trim()) {
    ...
    try {
      const newImageUrl = await editImage(bannerConfig.imageUrl, editPrompt);
      setBannerConfig(s => ({...s, imageUrl: newImageUrl}));
      setEditPrompt('');
    } catch (err) {
  ...
  }, [bannerConfig.imageUrl, editPrompt]);

  const handleGenerateCopy = useCallback(async () => {
    if (!url && (!imagePrompt || !bannerConfig.bannerTitle)) {
    ...
    try {
      const spanishCopy = url 
        ...
        : await generateCopyFromText(imagePrompt, bannerConfig.bannerTitle);
  ...
  }, [url, imagePrompt, bannerConfig.bannerTitle]);
  ...
  const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
   ...
   const handleLogoDragStart = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    dragStartRef.current = { startX: clientX, startY: clientY, logoX: bannerConfig.logoPosition.x, logoY: bannerConfig.logoPosition.y };
  }, [bannerConfig.logoPosition]);
  ...
  const handleLogoDragMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    setBannerConfig(s => ({...s, logoPosition: { x: newX, y: newY }}));
  }, [isDraggingLogo]);
  ...
  const handleTextDragStart = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    textDragStartRef.current = { startX: clientX, startY: clientY, textX: bannerConfig.textPosition.x, textY: bannerConfig.textPosition.y };
  }, [bannerConfig.textPosition]);
  ...
  const handleTextDragMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    setBannerConfig(s => ({...s, textPosition: { x: newX, y: newY }}));
  }, [isDraggingText]);
  ...
  const handleImageDragStart = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    if (!bannerConfig.imageUrl || isLoadingImage || isEditingImage) return;
    ...
    imageDragStartRef.current = { startX: clientX, startY: clientY, imageX: bannerConfig.imagePosition.x, imageY: bannerConfig.imagePosition.y };
  }, [bannerConfig.imageUrl, isLoadingImage, isEditingImage, bannerConfig.imagePosition]);
  ...
  const handleImageDragMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    setBannerConfig(s => ({...s, imagePosition: { x: newX, y: newY }}));
  }, [isDraggingImage]);
  ...
  const handleDownload = useCallback(async () => {
    ...
    if (!bannerElement || !bannerConfig.imageUrl) return;
    ...
    try {
        ...
        image.src = bannerConfig.imageUrl.includes('?') ? `${bannerConfig.imageUrl}&t=${new Date().getTime()}` : bannerConfig.imageUrl;
        ...
        image.onload = () => {
            ...
            if (imageRatio > canvasRatio) { 
                ...
                sx = (image.naturalWidth - sWidth) * (bannerConfig.imagePosition.x / 100);
            } else if (imageRatio < canvasRatio) { 
                sHeight = image.naturalWidth / canvasRatio;
                sy = (image.naturalHeight - sHeight) * (bannerConfig.imagePosition.y / 100);
            }
            
            ctx.filter = bannerConfig.activeFilter;
            ...
            const finalizeDownload = () => {
              ...
              link.href = dataUrl;
              const sanitizedTitle = bannerConfig.bannerTitle.replace(/[^a-z0-9\s]/gi, '').replace(/\s+/g, '_').toLowerCase();
            ...
            const drawTextElements = async () => {
                ...
                const titleMetrics = bannerConfig.showBannerTitle && bannerConfig.bannerTitle ? await getTextMetrics(bannerConfig.bannerTitle, bannerConfig.titleFontFamily, Math.max(16, bannerConfig.titleFontSize * scaleFactor), 800) : { lines: [], lineHeight: 0, totalHeight: 0 };
                const subheadlineMetrics = bannerConfig.showSubheadline && bannerConfig.bannerSubheadline ? await getTextMetrics(bannerConfig.bannerSubheadline, bannerConfig.subheadlineFontFamily, Math.max(12, bannerConfig.subheadlineFontSize * scaleFactor), 500) : { lines: [], lineHeight: 0, totalHeight: 0 };
                ...
                let blockCenterX = finalWidth * (bannerConfig.textPosition.x / 100);
                const blockCenterY = finalHeight * (bannerConfig.textPosition.y / 100);
                ...
                const drawElements = () => {
                    ...
                    const titleBlockWidth = bannerConfig.showBannerTitle && bannerConfig.bannerTitle ? getMaxLineWidth(titleMetrics.lines, bannerConfig.titleFontFamily, Math.max(16, bannerConfig.titleFontSize * scaleFactor), 800) : 0;
                    const subheadlineBlockWidth = bannerConfig.showSubheadline && bannerConfig.bannerSubheadline ? getMaxLineWidth(subheadlineMetrics.lines, bannerConfig.subheadlineFontFamily, Math.max(12, bannerConfig.subheadlineFontSize * scaleFactor), 500) : 0;
                    ...
                    const titleY = bannerConfig.subheadlinePosition === 'above' && subheadlineMetrics.totalHeight > 0 ? blockStartY + subheadlineMetrics.totalHeight + gap : blockStartY;
                    const subheadlineY = bannerConfig.subheadlinePosition === 'above' ? blockStartY : blockStartY + titleMetrics.totalHeight + gap;

                    if (bannerConfig.showBannerTitle && bannerConfig.bannerTitle) {
                        drawText(titleMetrics.lines, titleMetrics.lineHeight, drawX, titleY, bannerConfig.titleFontFamily, Math.max(16, bannerConfig.titleFontSize * scaleFactor), 800, bannerConfig.titleStyle, bannerConfig.textAlign, titleBlockWidth);
                    }
                    if (bannerConfig.showSubheadline && bannerConfig.bannerSubheadline) {
                        drawText(subheadlineMetrics.lines, subheadlineMetrics.lineHeight, drawX, subheadlineY, bannerConfig.subheadlineFontFamily, Math.max(12, bannerConfig.subheadlineFontSize * scaleFactor), 500, bannerConfig.subheadlineStyle, bannerConfig.textAlign, subheadlineBlockWidth);
                    }
                };
                drawElements();
            };
            ...
            const drawLogoAndFinalize = () => {
                if (bannerConfig.logoUrl) {
                    const logoImage = new Image();
                    logoImage.crossOrigin = 'anonymous';
                    logoImage.src = bannerConfig.logoUrl;
                    logoImage.onload = () => {
                        const logoBaseWidth = finalWidth * bannerConfig.logoScale;
                        ...
                        const logoX = finalWidth * bannerConfig.logoPosition.x - logoWidth;
                        const logoY = finalHeight * bannerConfig.logoPosition.y - logoHeight;
  ...
  }, [bannerConfig, aspectRatio, customDimensions]);
  ...
  const handleReset = useCallback(() => {
    ...
    setImagePrompt('');
    setEditPrompt('');
    setBannerConfig(initialBannerConfig);
    ...
    setCopiedField(null);
    setAspectRatio('1:1');
    setCustomDimensions({ width: 1080, height: 1080 });
    setSelectedLang('es');
    setCopyCache({});
    setHistory([]);
    setHistoryIndex(-1);
    ...
    <>
      <div className="grid grid-cols-2 gap-2 rounded-lg bg-slate-900/80 p-1 border border-slate-700 w-40">
          {(['Solid', 'Gradient'] as const).map(type => (
              <button key={type} type="button" onClick={() => setStyle({ ...style, type: type.toLowerCase() as ColorStyleType })} disabled={isLoading} className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${style.type === type.toLowerCase() ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-700/50'}`}>{type}</button>
          ))}
      </div>
      <div className="flex items-center gap-3">
        {style.type === 'solid' ? (
             <div className="flex items-center gap-3 animate-fade-in">
                  <div className="flex items-center gap-2">
                      {PRESET_COLORS.map((color) => (
                          <button key={color} type="button" onClick={() => setStyle({ ...style, color1: color })} className={`w-8 h-8 rounded-full border-2 transition-transform transform hover:scale-110 ${style.color1.toLowerCase() === color.toLowerCase() ? 'border-emerald-400 ring-2 ring-emerald-400 ring-offset-2 ring-offset-slate-800' : 'border-slate-600'}`} style={{ backgroundColor: color }} aria-label={`Set color to ${color}`} />
                      ))}
                  </div>
                  <div className="relative">
                      <input type="text" value={style.color1} onChange={(e) => setStyle({ ...style, color1: e.target.value })} className="w-28 bg-slate-900/80 border border-slate-700 rounded-lg py-1.5 pl-9 pr-2 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="#FFFFFF" />
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 animate-fade-in">
                <div className="flex items-center gap-2">
                    <input type="color" value={style.color1} onChange={(e) => setStyle({ ...style, color1: e.target.value })} className="w-10 h-10 bg-transparent border-none rounded-lg cursor-pointer" aria-label="Gradient start color" />
                    <input type="color" value={style.color2} onChange={(e) => setStyle({ ...style, color2: e.target.value })} className="w-10 h-10 bg-transparent border-none rounded-lg cursor-pointer" aria-label="Gradient end color" />
                </div>
                 <select value={style.direction} onChange={(e) => setStyle({ ...style, direction: e.target.value })} className="bg-slate-900/80 border border-slate-700 rounded-lg py-2 px-3 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
    ...
    <div className="min-h-screen bg-slate-900 font-sans text-slate-200 flex flex-col items-center p-4 sm:p-6 lg:p-8">
      <div className="w-full max-w-2xl mx-auto">
        ...
        <main>
                              ...
                              <div>
                                  ...
                                  <div className="grid grid-cols-3 gap-2 rounded-lg bg-slate-900/80 p-1 border border-slate-700">
                                      {(['Left', 'Center', 'Right'] as const).map((align) => {
                                          ...
                                          return <button key={align} type="button" onClick={() => setBannerConfig(s => ({...s, textAlign: alignValue}))} disabled={isLoading} className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 ${bannerConfig.textAlign === alignValue ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-700/50'}`} aria-pressed={bannerConfig.textAlign === alignValue}>{align}</button>;
                                      })}
                                  </div>
                              </div>
                              <div>
                                ...
                                <div className="flex flex-col items-start gap-3">
                                    {renderColorInputs(bannerConfig.titleStyle, (newStyle) => setBannerConfig(s => ({...s, titleStyle: newStyle})))}
                                </div>
                              </div>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                  <div>
                                      <label className="block text-sm font-medium text-slate-400 mb-2">Font Family</label>
                                      <select value={bannerConfig.titleFontFamily} onChange={(e) => setBannerConfig(s => ({...s, titleFontFamily: e.target.value}))} disabled={isLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-lg py-2 px-3 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                                  ...
                                  <div>
                                      ...
                                      <input type="number" value={bannerConfig.titleFontSize} onChange={(e) => setBannerConfig(s => ({...s, titleFontSize: parseInt(e.target.value, 10) || 0}))} disabled={isLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-lg py-2 px-3 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="64"/>
                                  </div>
                              </div>
                          </div>
                        )}
                         {bannerConfig.showSubheadline && (
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 rounded-lg bg-slate-900/80 p-1 border border-slate-700">
                                        {(['Above Title', 'Below Title'] as const).map((pos) => {
                                            ...
                                            return <button key={pos} type="button" onClick={() => setBannerConfig(s => ({...s, subheadlinePosition: positionValue}))} disabled={isLoading} className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-800 ${bannerConfig.subheadlinePosition === positionValue ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-700/50'}`} aria-pressed={bannerConfig.subheadlinePosition === positionValue}>{pos}</button>;
                                        })}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="flex flex-col items-start gap-3">
                                      {renderColorInputs(bannerConfig.subheadlineStyle, (newStyle) => setBannerConfig(s => ({...s, subheadlineStyle: newStyle})))}
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-400 mb-2">Font Family</label>
                                        <select value={bannerConfig.subheadlineFontFamily} onChange={(e) => setBannerConfig(s => ({...s, subheadlineFontFamily: e.target.value}))} disabled={isLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-lg py-2 px-3 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none">
                                            {FONT_OPTIONS.map(font => (<option key={font.name} value={font.family} style={{fontFamily: font.family}}>{font.name}</option>))}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-400 mb-2">Font Size (px)</label>
                                        <input type="number" value={bannerConfig.subheadlineFontSize} onChange={(e) => setBannerConfig(s => ({...s, subheadlineFontSize: parseInt(e.target.value, 10) || 0}))} disabled={isLoading} className="w-full bg-slate-900/80 border border-slate-700 rounded-lg py-2 px-3 text-slate-200 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="32" />
                                    </div>
                                </div>
                            </div>
                        )}
          ...
          {(isLoadingImage || bannerConfig.imageUrl) && (
            <div className="mt-6 bg-slate-800/50 rounded-xl shadow-lg p-6 backdrop-blur-sm border border-slate-700 animate-fade-in">
              ...
              <div 
                ref={bannerRef} 
                className={`${previewClassName} ${bannerConfig.imageUrl && !isLoadingImage && !isEditingImage ? 'cursor-grab' : ''} ${isDraggingImage ? 'active:cursor-grabbing' : ''}`} 
                style={previewStyle}
                ...
                {bannerConfig.imageUrl && (
                  <>
                    <img 
                      ...
                      style={{ 
                        objectPosition: `${bannerConfig.imagePosition.x}% ${bannerConfig.imagePosition.y}%`,
                        filter: bannerConfig.activeFilter 
                      }}
                    />
                    {(bannerConfig.showBannerTitle || bannerConfig.showSubheadline) && (
                         <div
                            ...
                            style={{
                                left: `${bannerConfig.textPosition.x}%`,
                                top: `${bannerConfig.textPosition.y}%`,
                                transform: 'translate(-50%, -50%)',
                                textAlign: bannerConfig.textAlign,
                                alignItems: bannerConfig.textAlign === 'left' ? 'flex-start' : bannerConfig.textAlign === 'right' ? 'flex-end' : 'center',
                            }}
                            onMouseDown={handleTextDragStart}
                            onTouchStart={handleTextDragStart}
                        >
                            {bannerConfig.subheadlinePosition === 'above' && bannerConfig.showSubheadline && bannerConfig.bannerSubheadline && (
                                <p className="font-medium leading-tight" style={{ ...getPreviewTextStyle(bannerConfig.subheadlineStyle, bannerConfig.subheadlineFontFamily, bannerConfig.subheadlineFontSize), textShadow: '0px 1px 3px rgba(0,0,0,0.7), 0px 2px 8px rgba(0,0,0,0.6)' }}>
                                    {bannerConfig.bannerSubheadline}
                                </p>
                            )}
                            {bannerConfig.showBannerTitle && bannerConfig.bannerTitle && (
                                <h3 className={`font-extrabold leading-tight w-full max-w-full break-words`} style={{ ...getPreviewTextStyle(bannerConfig.titleStyle, bannerConfig.titleFontFamily, bannerConfig.titleFontSize), textShadow: '0px 2px 5px rgba(0,0,0,0.7), 0px 5px 20px rgba(0,0,0,0.6)' }}>
                                    {bannerConfig.bannerTitle}
                                </h3>
                            )}
                            {bannerConfig.subheadlinePosition === 'below' && bannerConfig.showSubheadline && bannerConfig.bannerSubheadline && (
                                 <p className="font-medium leading-tight" style={{ ...getPreviewTextStyle(bannerConfig.subheadlineStyle, bannerConfig.subheadlineFontFamily, bannerConfig.subheadlineFontSize), textShadow: '0px 1px 3px rgba(0,0,0,0.7), 0px 2px 8px rgba(0,0,0,0.6)' }}>
                                    {bannerConfig.bannerSubheadline}
                                 </p>
                            )}
                     ...
                     {bannerConfig.logoUrl && (
                        <img ref={logoRef} src={bannerConfig.logoUrl} alt="Uploaded Logo" className="absolute h-auto cursor-grab active:cursor-grabbing z-10" style={{ width: `${bannerConfig.logoScale * 100}%`, left: `calc(${bannerConfig.logoPosition.x * 100}% - ${logoRef.current?.offsetWidth ?? 0}px)`, top: `calc(${bannerConfig.logoPosition.y * 100}% - ${logoRef.current?.offsetHeight ?? 0}px)` }} onMouseDown={handleLogoDragStart} onTouchStart={handleLogoDragStart} />
                    )}
                  </>
                )}
              </div>
               {bannerConfig.imageUrl && !isLoadingImage && (
                 <>
                  <p className="text-center text-sm text-slate-400 mt-4 italic">Tip: Drag the image background, text block, or logo to adjust their positions.</p>
                   ...
                   <div className="mt-4 border-t border-slate-700 pt-6">
                     ...
                     <div className="flex flex-wrap gap-2">
                        {FILTERS.map(filter => (
                            <button key={filter.name} onClick={() => setBannerConfig(s => ({...s, activeFilter: filter.value}))} className={`px-3 py-1.5 text-sm font-medium rounded-md border-2 transition-all ${bannerConfig.activeFilter === filter.value ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-slate-700/50 border-slate-600 text-slate-300 hover:border-slate-500'}`}>
                                {filter.name}
                            </button>
                        ))}
                     </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {bannerConfig.logoUrl && (
                                <div className="flex items-center gap-2 p-2 bg-slate-900/80 rounded-lg border border-slate-700">
                                    <img src={bannerConfig.logoUrl} alt="Logo preview" className="w-10 h-10 object-contain rounded"/>
                                    <button onClick={() => { setBannerConfig(s => ({...s, logoUrl: null})); if(logoInputRef.current) logoInputRef.current.value = ''; }} className="text-xs text-red-400 hover:text-red-300 font-semibold" aria-label="Remove logo">Remove</button>
                                </div>
                            )}
                        </div>
                        {bannerConfig.logoUrl && (
                           <div className="animate-fade-in">
                             ...
                             <input id="logo-size" type="range" min="0.05" max="0.5" step="0.01" value={bannerConfig.logoScale} onChange={(e) => setBannerConfig(s => ({...s, logoScale: parseFloat(e.target.value)}))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer" disabled={isLoading} />
                           </div>
                        )}
                      </div>
                  </div>
                 <div className="flex items-center justify-center gap-4 mt-6 border-t border-slate-700 pt-6">
                    <button onClick={handleUndo} disabled={!canUndo} className="flex items-center justify-center gap-2 text-slate-300 font-semibold p-3 rounded-lg hover:bg-slate-700/60 disabled:text-slate-600 disabled:bg-transparent disabled:cursor-not-allowed transition-colors" aria-label="Undo last change">
                        <UndoIcon className="w-5 h-5" /> Undo
                    </button>
                     <button onClick={handleRedo} disabled={!canRedo} className="flex items-center justify-center gap-2 text-slate-300 font-semibold p-3 rounded-lg hover:bg-slate-700/60 disabled:text-slate-600 disabled:bg-transparent disabled:cursor-not-allowed transition-colors" aria-label="Redo last change">
        ...
        <div className="mt-8 py-4 text-center border-t border-slate-700/50">
            <button
                onClick={handleReset}
                className="flex items-center justify-center gap-2 mx-auto text-slate-400 font-semibold py-2 px-4 rounded-lg hover:bg-slate-800 hover:text-slate-200 disabled:opacity-50 transition-colors duration-300"
                disabled={isLoading}
